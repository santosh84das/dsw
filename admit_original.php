...
require('fpdf.php');
if(date('YmdHm')>201912042359  || date('YmdHm')<201811021030)
{
	?>
<script>
...
if(isset($_REQUEST['can']) && isset($_REQUEST['cd']))
{
		...
		$data_reg=mysql_query("select * from online_app_zp_ttb where cand_app_no='".$cand_app_no."' and cand_dob='".$cand_dob."' and comp_test=1");
		mysql_close($link);
...
$pdf->SetFont('Arial','B',$fntsz-1);
$pdf->Cell($linwp,5,'ADMIT CARD FOR  COMPUTER EXAMINATION/TEST & PERSONALITY TEST  FOR ENGAGEMENT TO THE POST OF DATA ENTRY OPERATOR, ',0,1,'C');
$pdf->Cell($linwp,2,'FOR RUPASHREE PRAKALPA ON CONTRACTUAL BASIS.',0,1,'C');
...
$pdf->MultiCell($linwp,3,'1. Candidates should check the e-Admit card carefully and bring discrepancies, if any, to the notice of the authority through email id (dpmu.n24g@gmail.com) on 10th December, 2019 upto 3PM.',0,'L');
...
$pdf->MultiCell($linwp,3,'6. Candidate shall bring the following documents in original : Qualification documents, Markesheets, academic certificates, Experience certificate, proof of date of birth, caste certificate or any other document which is mandatory for the post. ',0,'L');

$pdf->MultiCell($linwp,4,'7. CARRYING OF MOBILE PHONE / CALCULATOR OR ANY SORT OF ELECTRONIC GADGET / CORRECTION FLUID / BAG WILL NOT BE PERMITTED WITHIN THE EXAMINATION HALL. AUTHORITY WILL HAVE NO RESPONSIBILITY FOR CUSTODY OF SUCH ARTICLES.',0,'L');
$pdf->Cell($linwp,3,'8. No TA/DA will be admissible for appearing for the written examination / computer test / interview.',0,1,'L');
...
$pdf->MultiCell($linwp,3,'9. Candidate should not sign in advance  in the lower portion of the Admit Card (specified for Office use). This has to be done at the time of examination in presence of the invigilator only. The photo and the signature in the admit card should be same as those of the application form.',0,'L');
$pdf->MultiCell($linwp,3,'10. There will be no negative marking system',0,'L');
$pdf->MultiCell($linwp,3,'11. Admission to the Examination Hall will not be allowed after 10 (ten) minutes of the scheduled time ie. commencement of exam. ',0,'L');
$pdf->MultiCell($linwp,3,'12. No candidate will be allowed to leave examination hall before full expiry of the scheduled hour of the conclusion of the examination',0,'L');
$pdf->MultiCell($linwp,3,'13. No request of change of venue of examination will be entertained.',0,'L');
...
$pdf->Cell($linwp/5,3,'ADDITIONAL DISTRICT MAGISTRATE (TRY)',0,1,'C');

$pdf->SetXY($get_x+110,$get_y+17);
$pdf->Cell($linwp/5,3,'&',0,1,'C');

$pdf->SetXY($get_x+110,$get_y+20);
$pdf->Cell($linwp/5,3,'CHAIRPERSON, DISTRICT LEVEL SELECTION COMMITTEE',0,1,'C');
