<?php
session_start();
if(date('YmdHm')>202204032359  || date('YmdHm')<202203131030)
{
	?>
<script>
...
if(isset($_POST['sub_sub']) && $_POST['sub_sub']=='Submit'){
	...
	if($check_page!=$frm_data){
		...
		$data_r=mysql_query("select cand_app_no from online_app_zp_ttb where cand_app_no='".$cand_app_no."' and cand_dob='".$cand_dob."'");
		...
		mysql_query("update online_app_zp_ttb set print_chk='y', print_dt='".date("Y-m-d")."', print_time='".date("h:i:s")."', print_ip='".$_SERVER['REMOTE_ADDR']."' where cand_app_no='".$data_reg[0]."'");
		mysql_close($link);
		?>
    	<script>
		...
		<div class="container" style="height:600px">
            ...
            <div class="row">
                <div class="col-lg-12 text-center">
                    ...
                    <h3 class="section-subheading text-muted center">ADMIT CARD FOR WRITTEN EXAMINATION FOR ENGAGEMENT TO THE POST OF BENCH CLERK & LOWER DIVISION CLERK <br/> ON CONTRACTUAL BASIS IN JUVENILE JUSTICE BOARD ,NORTH 24 PARGANAS  </h3>
                </div>
            </div>
       		...
       		<form action="" method="post" name="app_print" onSubmit="return ckfrm()">
                        <table width="366" align="center" cellspacing="10">
                          <tr>
                           ...
                           <tr><td>&nbsp;&nbsp;</td><td>&nbsp;&nbsp;</td></tr>
						   <tr><td style="font-size:13px; font-weight: 600;">Date of Birth (DD/MM/YYYY)</td><td><input type="text" maxlength="10" style="font-size:12px; width:80px;" name="cand_dob" id="cand_dob" onKeyPress="return dt_allow('sub_sub', event)" autocomplete="off" value="<?php if(isset($_POST['sub_sub'])){echo $_POST['cand_dob'];}?>" />
