...
function checkdate_range(){
	...
	if(parseInt(dto)<19800101 || parseInt(dto)>19990101){
		alert("Candidates should be between 18 to 40 yrs, as on 01/01/2022");
		inputdate.focus();
		inputdate.value='';
	}
}
...
function date_range(min_age,max_age){
	...
	//alert('19750501-----------'+parseInt(dto)+'-----------19940501');
	var lim=0;
	lim_max=20220101-parseInt(max_age)*10000;
	lim_min=20220101-parseInt(min_age)*10000;
	
	
	if( parseInt(dto)<parseInt(lim_max) || parseInt(dto)>parseInt(lim_min) ){


		alert("Candidates should be between "+ min_age +" to "+ max_age +" years as on 01/01/2022");
		inputdate.focus();
		inputdate.value='';
	...
	var age = y1 - y2;
	document.o_a.cand_age.value= y1 - y2-1; 
...
if(x<=0)
{
alert("Year of experience must be grater than zero year.");
val.value='';
val.focus();
return false;
}
...
function isDate(inputStr)
{
    ...
    var re=/^(([0-2][0-9])|(3[0|1]))\/((0[1-9])|(1[0-2]))\/\d{4}$/;
	...
	if(re.test(str)==false) {
		...
		{
			var datePart = str.substr(0, 2);
            var monPart = str.substr(3, 2);
            var yearPart = str.substr(6, 4);

			//alert(datePart+" : "+monPart+" : "+yearPart);
            switch(monPart)
			{
			case "04", "06","09", "11":
				if(datePart > 30) 
	...
	{
		inputStr.value='';
		alert("Not a valid Date. Please enter valid date");
		return false;
	}
	return true;
}
