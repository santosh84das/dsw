...
if(isset($_POST['sub_sub']) && $_POST['sub_sub']=='Submit'){
	...
	$cand_data=mysql_fetch_array(mysql_query("select r_no from online_app_zp_f_ttb where r_no='".$r_no."'"));
	if($cand_data[0]==''){
		?>
		<script>
	...
	?>
    	<script>
		window.location='li_print.php?r_no=<?php echo $r_no;?>';
		</script>
  		<?php
		exit();
	}
...
<div class="wrapper">
	<div class="main">
		...
		<div class="container">
			...
			<div class="midright">
            	<div class="mid_s">
                	<p align="center" class="stylevr">Print Letter of intimation</p>
                    
           	  <p align="center" class="stylevr" style="padding-left:50px">
                   		<form action="" method="post" name="app_print" onSubmit="return ckfrm()">
                        <table>
                          <tr>
                       		<td>Roll no</td><td><input type="text" maxlength="6" style="font-size:12px; width:180px;" name="r_no" id="r_no" onChange="javascript:this.value=this.value.toUpperCase();" onKeyPress="return number_allow('sub_sub', event)" autocomplete="off" value="<?php if(isset($_POST['sub_sub'])){echo $_POST['r_no'];}?>" /> </td></tr>
